...
import { useDashboardMainScroll } from "~/components/dashboard/dashboard-main-scroll-context"
...
export function AssetDetailSheet({
  ...
  const { scrollEl: mainScrollEl, portalEl: mainPortalEl } = useDashboardMainScroll()
  const useDockedDetail = mainPortalEl !== null && mainScrollEl !== null

  // Note: We've removed the manual overflow: hidden lock on mainScrollEl to prevent 
  // 'scroll stuttering' or 'stuck' feelings when opening/closing the docked sheet.
  // The background remains interactive but the focus is managed by the sheet.
  ...
  if (useDockedDetail && mainPortalEl) {
    return createPortal(
      <AnimatePresence>
        {selectedAsset && (
          <div className={cn(
            "absolute inset-0 z-10 flex min-h-0 min-w-0 flex-row justify-end overflow-hidden",
            !selectedAsset && "pointer-events-none"
          )}>
            <motion.button
              type="button"
              initial={{ opacity: 0 }}
