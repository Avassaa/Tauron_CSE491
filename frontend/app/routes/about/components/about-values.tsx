    ...
    <section className="py-12 relative z-10">
      <div className="flex flex-col items-center mb-10 text-center">
        <h2 className="text-3xl md:text-5xl font-bold text-foreground dark:text-white mb-4 tracking-tight">
          Our <span className="text-meta-blue">Core Values</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {values.map((value) => (
          <div
            key={value.index}
            className="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-sm transition-all duration-500 hover:border-border hover:bg-muted/50 dark:border-white/5 dark:bg-white/[0.02] dark:shadow-none dark:hover:border-white/10 dark:hover:bg-white/[0.04]"
          >
            ...
            <div>
              <div className="flex justify-between items-start mb-5">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-muted transition-transform duration-500 group-hover:scale-110 dark:border-white/10 dark:bg-neutral-950">
                  {value.icon}
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground border border-border px-2 py-1 rounded bg-muted dark:border-white/5 dark:bg-neutral-950/50 dark:text-neutral-500">
                  {value.tag}
                </span>
              </div>
              ...
              <p className="text-sm leading-relaxed text-muted-foreground transition-colors dark:text-neutral-400 dark:group-hover:text-neutral-300">
                {value.description}
              </p>
            </div>

            <div className="mt-6 pt-4 border-t border-border dark:border-white/5 flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground dark:text-neutral-600">
                Ethical Standard
              </span>
